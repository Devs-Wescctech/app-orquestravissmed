...
async function testUpdate() {
    ...
    try {
        ...
        async function tryPatch(bodyObj) {
            out += `\nTrying: ${JSON.stringify(bodyObj)}\n`;
            const patchRes = await fetch(addrUrl, { method: 'PATCH', headers, body: JSON.stringify(bodyObj) });
            out += `PATCH Status: ${patchRes.status}\n`;
            if (patchRes.status !== 500 && patchRes.status !== 405) {
                out += `Response: ${(await patchRes.text()).substring(0, 500)}\n`;
            }
        }

        await tryPatch({ name: 'Test Name', insuranceSupport: [] });
        await tryPatch({ name: 'Test Name', insuranceSupport: false });
        await tryPatch({ name: 'Test Name', insuranceSupport: [1] });
        await tryPatch({ name: 'Test Name', insurance_support: [] });

        fs.writeFileSync('doctoralia_test_out.txt', out);
        console.log("Done");
    } catch (e) {
        console.error(e);
    }
