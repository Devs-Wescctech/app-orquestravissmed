...
export async function reconcileManagedRemoval(input: {
  ...
  try {
    ...
    if (
      !emptyComplete(await input.client.getBookings(f, d, a, start, end)) ||
      !emptyComplete(await input.client.getCalendarBreaks(f, d, a, start, end))
    )
      return false;
    if (!snapshotMatches(await read(), plan.before)) return false;
    if (
      !(await input.authorized()) ||
      !snapshotMatches(await read(), plan.before)
    )
      return false;
