...
import { consultationRecords, isConsultationRecord, isVismedConsultation, canSynchronizeConsultation, preserveAppointmentMetadata } from './consultation-policy';
...
export class BookingSyncService implements OnModuleInit, OnModuleDestroy {
                    ...
                    for (const a of agendamentos) {
                        ...
                        try {
                            if (!isVismedConsultation(a)) {
                                await this.preserveExcludedAppointment(conn.clinicId, a);
                                this.logger.debug(`[VISMED-POLL] clinicId=${conn.clinicId} appointmentId=${vid} skipped=not_confirmed_consultation`);
                                continue;
                            }
    ...
    private async reconcileDisappearedFromVismed(
        ...
        const confirmedAppointments = new Map<string, any>();
        for (const u of units) {
            try {
                ...
                for (const a of ags) {
                    ...
                    if (vid) {
                        confirmSeen.add(vid);
                        confirmedAppointments.set(vid, a);
                    }
                }
            } catch (err: any) {
        ...
        for (const rec of confirmedGone) {
            try {
                ...
                // reconfirmação. Ambiguidade/erro são fail-safe: não cancelamos.
                const replacement = await this.tryAdoptVismedReplacement(
                    rec,
                    confirmSeen,
                ...
                // Absence in a filtered list is not cancellation. Rebind remains
                // possible above; destructive cancellation requires individual proof.
                const currentResponse = await this.vismedService.getAgendamentoById(rec.vismedAppointmentId, baseUrl);
                const current = Array.isArray(currentResponse) && currentResponse.length === 1 ? currentResponse[0] : null;
     ...
     * doctoraliaBookingId e evitando cancelar na Doctoralia. A presença do
     * candidato vem obrigatoriamente da reconfirmação atual, nunca apenas da
     * base local.
     */
    private async tryAdoptVismedReplacement(rec: {
        ...
        const targetFor = (original: typeof rec): Date =>
            original.lastMoveBy === 'VISMED' && original.lastMoveTargetStartAt
                ? original.lastMoveTargetStartAt
                : original.startAt;
        const target = targetFor(rec);
        // Candidato precisa ser recente: um replacement chega pelo poll atual ou
        // imediatamente anterior; não adotamos um registro antigo coincidente.
        const freshnessMs = 10 * 60 * 1000;
        const createdAfter = new Date(Date.now() - freshnessMs);
        const oldVid = rec.vismedAppointmentId;

        const normPhone = (phone: any): string => {
            const digits = String(phone || '').replace(/\D/g, '');
            return digits.length >= 9 ? digits.slice(-9) : '';
        };
        const firstStrongPhone = (...phones: any[]): string =>
            phones.map(normPhone).find(Boolean) || '';
        const normName = (name: any): string => {
            const normalized = this.normalizeName(String(name || ''));
            return normalized.length >= 4 && !/^paciente\b/.test(normalized) ? normalized : '';
        };
        const parseConfirmedStart = (confirmed: any): Date | null => {
            const date = String(confirmed?.dataagendamento || '').trim();
            const time = String(confirmed?.horarioagendamento || '').trim().slice(0, 5);
            if (!date || !/^\d{2}:\d{2}$/.test(time)) return null;
            const parsed = new Date(`${date}T${time}:00-03:00`);
            return isNaN(parsed.getTime()) ? null : parsed;
        };
        const parseConfirmedEnd = (confirmed: any, currentStart: Date): Date => {
            const date = String(confirmed?.dataagendamento || '').trim();
            const time = String(confirmed?.horarioagendamentofinal || '').trim().slice(0, 5);
            if (date && /^\d{2}:\d{2}$/.test(time)) {
                const parsed = new Date(`${date}T${time}:00-03:00`);
                if (!isNaN(parsed.getTime()) && parsed.getTime() > currentStart.getTime()) return parsed;
            }
            return new Date(currentStart.getTime() + 30 * 60 * 1000);
        };
        const isTruthyVismedFlag = (value: any): boolean =>
            value === true || value === 1 || String(value) === '1';
        const evaluateCandidate = (candidate: any, original: typeof rec) => {
            const candidateId = candidate?.vismedAppointmentId;
            const currentTarget = targetFor(original);
            if (
                !candidateId ||
                candidateId === original.vismedAppointmentId ||
                !confirmSeen.has(candidateId) ||
                candidate.clinicId !== original.clinicId ||
                candidate.vismedDoctorId !== original.vismedDoctorId ||
                (candidate.status !== 'BOOKED' && candidate.status !== 'CONFIRMED') ||
                candidate.createdAt.getTime() < createdAfter.getTime() ||
                Math.abs(candidate.startAt.getTime() - currentTarget.getTime()) > tolMs
            ) {
                return null;
            }

            const confirmed = confirmedAppointments.get(candidateId);
            const confirmedDoctorId = confirmed?.idprofissional !== undefined &&
                confirmed?.idprofissional !== null
                ? String(confirmed.idprofissional)
                : '';
            const candidateDoctorId = candidate?.rawPayload?.idprofissional !== undefined &&
                candidate?.rawPayload?.idprofissional !== null
                ? String(candidate.rawPayload.idprofissional)
                : '';
            const currentStart = parseConfirmedStart(confirmed);
            if (
                !confirmed ||
                !isVismedConsultation(confirmed) ||
                !isVismedConsultation(candidate.rawPayload) ||
                !confirmedDoctorId ||
                !candidateDoctorId ||
                confirmedDoctorId !== candidateDoctorId ||
                !currentStart ||
                Math.abs(currentStart.getTime() - currentTarget.getTime()) > tolMs ||
                isTruthyVismedFlag(confirmed.cancelado) ||
                isTruthyVismedFlag(confirmed.naocompareceu)
            ) {
                return null;
            }

            const originalPhone = normPhone(original.patientPhone);
            const originalName = normName(original.patientName);
            if (!originalPhone && !originalName) return null;

            const candidatePhone = firstStrongPhone(
                confirmed.telefonepaciente,
                confirmed.celularpaciente,
                confirmed.telefone1,
            );
            const candidateName = normName(confirmed.nomepaciente || confirmed.nome);
            const patientMatches = originalPhone || candidatePhone
                ? !!originalPhone && !!candidatePhone && originalPhone === candidatePhone
                : !!originalName && !!candidateName && originalName === candidateName;
            if (!patientMatches) return null;

            const currentStatus: 'BOOKED' | 'CONFIRMED' =
                isTruthyVismedFlag(confirmed.confirmado) ? 'CONFIRMED' : 'BOOKED';
            return {
                record: candidate,
                confirmed,
                currentStart,
                currentEnd: parseConfirmedEnd(confirmed, currentStart),
                currentStatus,
            };
        };

        if (!normPhone(rec.patientPhone) && !normName(rec.patientName)) {
            this.logger.debug(
                `[RECONCILE-DISAPPEARED] adopt-skip vismedApptId=${oldVid} — sem fingerprint forte de paciente`,
            );
            return 'absent';
        }

        let candidates: Array<NonNullable<ReturnType<typeof evaluateCandidate>>>;
        try {
            const possibleCandidates = await this.prisma.bookingSync.findMany({
                where: {
            ...
            candidates = possibleCandidates
                .map(candidate => evaluateCandidate(candidate, rec))
                .filter((candidate): candidate is NonNullable<typeof candidate> => candidate !== null);
        } catch (err: any) {
            await this.persistVismedReplacementDiagnostic(
                rec,
                `rebind VisMed: falha ao selecionar replacement (${String(err?.message || 'erro').slice(0, 160)})`,
            );
            this.logger.warn(
                `[RECONCILE-DISAPPEARED] replacement selection falhou para ${oldVid}: ${err.message}`,
            );
            return 'error';
        }
        ...
        if (candidates.length > 1) {
            const candidateIds = candidates.map(c => c.record.id).join(',');
            await this.persistVismedReplacementDiagnostic(
                rec,
                `rebind VisMed ambíguo: ${candidates.length} candidatos confirmados (${candidateIds})`,
            );
            this.logger.warn(
                `[RECONCILE-DISAPPEARED] REPLACEMENT_AMBIGUOUS clinicId=${rec.clinicId} oldVismedApptId=${oldVid} candidateBookingSyncIds=${candidateIds}`,
            );
            return 'ambiguous';
        }

        const candidate = candidates[0];
        const newVid = candidate.record.vismedAppointmentId;
        if (candidate.record.doctoraliaBookingId || candidate.record.doctoraliaBreakId) {
            await this.persistVismedReplacementDiagnostic(
                rec,
                `rebind VisMed bloqueado: candidato ${candidate.record.id} possui vínculo Doctoralia`,
            );
            this.logger.warn(
                `[RECONCILE-DISAPPEARED] REPLACEMENT_COLLISION clinicId=${rec.clinicId} oldVismedApptId=${oldVid} candidateBookingSyncId=${candidate.record.id}`,
            );
            return 'error';
        }
        ...
        try {
            await this.prisma.$transaction(async tx => {
                // Lock pessimista das duas linhas antes da releitura. Sem FOR UPDATE,
                // outro poll poderia vincular/alterar o candidato entre findUnique e
                // delete, mesmo dentro da transação. A ordenação reduz risco de
                // deadlock quando duas reconciliações concorrentes tocam os mesmos IDs.
                await tx.$queryRaw`
                    SELECT "id"
                    FROM "BookingSync"
                    WHERE "id" IN (${rec.id}, ${candidate.record.id})
                    ORDER BY "id"
                    FOR UPDATE
                `;
                ...
                if (cand.doctoraliaBookingId || cand.doctoraliaBreakId) {
                    throw new Error('candidate ganhou vínculo Doctoralia');
                }
                ...
                await tx.bookingSync.update({
                    ...
                    data: {
                        ...
                        startAt: revalidated.currentStart,
                        endAt: revalidated.currentEnd,
                        duration: Math.max(
                            5,
                            Math.round(
                                (revalidated.currentEnd.getTime() - revalidated.currentStart.getTime()) / 60000,
                            ),
                        ),
                        patientName: String(
                            revalidated.confirmed.nomepaciente ||
                            revalidated.confirmed.nome ||
                            cand.patientName,
                        ).slice(0, 200),
                        patientSurname: cand.patientSurname,
                        patientPhone:
                            revalidated.confirmed.telefonepaciente ||
                            revalidated.confirmed.celularpaciente ||
                            revalidated.confirmed.telefone1 ||
                            cand.patientPhone,
                        patientEmail: cand.patientEmail,
                        patientCpf: cand.patientCpf,
                        patientBirthDate: cand.patientBirthDate,
                        patientGender: cand.patientGender,
                        rawPayload: revalidated.confirmed,
                        syncedToVismed: true,
                        syncError: null,
        ...
        this.logger.log(
            `[RECONCILE-DISAPPEARED] ADOPTED replacement: BookingSync ${rec.id} ` +
                `vismedApptId ${oldVid} → ${newVid} @ ${candidate.currentStart.toISOString()}`,
        );
        return 'adopted';
    }

    private async persistVismedReplacementDiagnostic(
        rec: { id: string; vismedAppointmentId: string | null },
        diagnostic: string,
    ): Promise<void> {
        try {
            await this.prisma.bookingSync.updateMany({
                ...
                data: { syncError: diagnostic.slice(0, 500) },
            });
        } catch (err: any) {
            this.logger.warn(
                `[RECONCILE-DISAPPEARED] não foi possível persistir diagnóstico do replacement para ${rec.vismedAppointmentId}: ${err.message}`,
            );
        }
    }
    ...
    private async tryAdoptDoctoraliaReplacement(
        ...
        const eligibleActive: any[] = [];
        for (const booking of liveActive) {
            if (await isConsultationRecord(this.prisma, { clinicId: rec.clinicId, rawPayload: { data: { visit_booking: booking } } })) eligibleActive.push(booking);
        }
        const tolMs = 2 * 60 * 1000;
        ...
        const candidate = eligibleActive.find((b: any) => {
                ...
                for (const addr of addresses) {
                    ...
                    try {
                        ...
                        for (const booking of bookings) {
                            ...
                            if (match) {
                                if (!await isConsultationRecord(this.prisma, { clinicId, rawPayload: { data: { visit_booking: booking } } })) continue;
    ...
    private async preserveExcludedAppointment(clinicId: string, a: any): Promise<void> {
        if (!a?.idpacienteagendamento) return;
        // No new exam row. Retain the latest authoritative payload on existing
        // rows so a consultation reclassified as an exam cannot be replayed later.
        await this.prisma.bookingSync.updateMany({
            where: { clinicId, vismedAppointmentId: String(a.idpacienteagendamento) },
            data: { rawPayload: a },
        });
    }
    ...
    private async upsertVismedAppointment(
        ...
        if (!isVismedConsultation(a)) {
            await this.preserveExcludedAppointment(clinicId, a);
            return false;
        }
    ...
    private async propagateVismedCancellationToDoctoralia(syncId: string): Promise<void> {
        ...
        if (!rec) return;
        if (!await canSynchronizeConsultation(this.prisma, rec)) return;
    ...
    private async propagateDoctoraliaRescheduleToVismed(syncId: string, previousVismedAppointmentId: string | null): Promise<void> {
        ...
        const fakeBooking: any = {
            ...
            address_service: { id: (rec.rawPayload as any)?.data?.new_visit_booking?.address_service?.id
                || (rec.rawPayload as any)?.data?.visit_booking?.address_service?.id || rec.addressServiceId },
    ...
    async getSkippedBookingAlerts(clinicId: string) {
        const allPending = await this.prisma.skippedBookingAlert.findMany({
            where: { clinicId, resolved: false, startAt: { gte: new Date() } },
            orderBy: { startAt: 'desc' },
        });
        const pendingRecords = allPending.length ? await consultationRecords(this.prisma, await this.prisma.bookingSync.findMany({
            where: { clinicId, id: { in: allPending.map(a => a.bookingSyncId) } },
            select: { id: true, clinicId: true, rawPayload: true, addressServiceId: true },
        })) : [];
        const pendingIds = new Set(pendingRecords.map(r => r.id));
        const pending = allPending.filter(a => pendingIds.has(a.bookingSyncId));
    ...
    private async syncDoctoraliaBreak(bookingSyncId: string): Promise<void> {
        ...
        const ownership = new CalendarBreakOwnership(this.prisma);
        const scope = { clinicId: rec.clinicId, facilityId: String(facilityId), doctorId: String(mapping.externalId), addressId };
        const pending = async (error: Error): Promise<never> => {
            await this.prisma.bookingSync.update({ where: { id: rec.id }, data: {
                syncedToDoctoralia: false, syncError: error.message.slice(0, 500),
            } });
            throw error;
        };
        ...
        if (!isActive) {
            ...
            await this.prisma.bookingSync.update({
                where: { id: rec.id },
                data: { doctoraliaBreakId: null, doctoraliaFacilityId: facilityId, doctoraliaAddressId: addressId, syncedToDoctoralia: true, syncError: null },
            });
            await ownership.clear(rec.id);
            return;
        }
        ...
        if (rec.doctoraliaBreakId) {
            try {
                ...
                await this.prisma.bookingSync.update({
                    where: { id: rec.id },
                    data: { syncedToDoctoralia: true, syncError: null },
                });
                ...
                await this.prisma.bookingSync.update({ where: { id: rec.id }, data: { doctoraliaBreakId: null } });
                await ownership.clear(rec.id);
            }
        }

        const receipt = await ownership.read(rec.id);
        const persist = async (breakId: string, synced = true) => {
            await ownership.requireOwned(rec.id, scope, breakId);
            await this.prisma.bookingSync.update({ where: { id: rec.id }, data: {
                doctoraliaBreakId: breakId, doctoraliaFacilityId: scope.facilityId,
                doctoraliaDoctorId: scope.doctorId, doctoraliaAddressId: addressId,
                syncedToDoctoralia: synced, syncError: null,
            } });
        };
        if (receipt) {
            if (ownership.matches(receipt, scope)) {
                // Recover a confirmed POST whose subsequent BookingSync update failed.
                await persist(receipt.breakId, false);
                return;
            }
            return pending(new Error('BREAK_CREATION_UNCONFIRMED: uma tentativa anterior não teve confirmação de criação. Não foi associado nem removido bloqueio; conferir na Doctoralia antes de repetir.'));
        }
        await this.rateLimiter.acquire('doctoralia');
        try { await ownership.begin(rec.id, scope); }
        catch (error: any) {
            if (error?.code === 'P2002') return pending(new Error('BREAK_CREATION_UNCONFIRMED: já existe uma tentativa em andamento ou aguardando conferência.'));
            throw error;
        }
        let created: any;
        try {
            created = await client.addCalendarBreak(facilityId, mapping.externalId, addressId, { since, till });
        } catch (err: any) {
            // Definite rejection / pre-send failure may be retried. A 409 NEVER identifies an owner.
            if (isPreSendFailure(err) || (typeof err?.status === 'number' && err.status >= 400 && err.status < 500) || isConflict(err)) {
                await ownership.clear(rec.id);
            }
            const message = isConflict(err)
                ? 'BREAK_CONFLICT: a Doctoralia recusou o bloqueio por conflito. Um horário igual não comprova que o bloqueio pertence a este agendamento; nenhum bloqueio foi associado.'
                : isPreSendFailure(err)
                    ? 'BREAK_RETRY_PENDING: envio não iniciado; será tentado novamente.'
                    : 'BREAK_CREATION_UNCONFIRMED: criação não confirmada pela Doctoralia. Nenhum bloqueio foi associado; conferir o resultado antes de repetir.';
            return pending(new Error(message));
        }
        const breakId = (typeof created?.id === 'string' && created.id.trim())
            || (typeof created?.id === 'number' && Number.isSafeInteger(created.id) && created.id > 0 ? String(created.id) : null);
        if (!breakId) return pending(new Error('BREAK_CREATION_UNCONFIRMED: a resposta não informou o identificador do bloqueio criado; conferir antes de repetir.'));
        await ownership.confirm(rec.id, scope, breakId);
        await persist(breakId);
        ...
        try {
            return await this.bookingClaimService.withClaim(
                ...
                async (claimSignal) => {
                    ...
                    if (reserved.rawPayload && !await canSynchronizeConsultation(this.prisma, reserved)) {
                        return { processed: false, reason: 'not_confirmed_consultation' };
                    }
    ...
    private async handleBookingCanceled(clinicId: string, data: any, rawNotification: any) {
        ...
        if (existing) {
            const updated = await this.prisma.bookingSync.update({
                ...
                data: {
                    status: 'CANCELLED',
                    rawPayload: preserveAppointmentMetadata(rawNotification, existing),
                    processedAt: new Date(),
                },
            });
            syncId = updated.id;
    ...
    private async handleBookingMoved(clinicId: string, data: any, rawNotification: any) {
        ...
        if (booking?.address_service?.id && !await isConsultationRecord(this.prisma, {
            clinicId, rawPayload: { data: { visit_booking: booking } },
        })) return { processed: false, reason: 'not_confirmed_consultation' };
        ...
        if (!existing && oldBooking?.start_at) {
            ...
            if (candidate && await canSynchronizeConsultation(this.prisma, candidate)) {
                this.logger.log(
        ...
        if (existing.startAt.getTime() === newStartAt.getTime()) {
            await this.prisma.bookingSync.update({
                where: { id: existing.id },
                data: { rawPayload: preserveAppointmentMetadata(rawNotification, existing), processedAt: new Date() },
            });
            ...
            for (const u of units) {
                ...
                for (const a of agendamentos) {
                    if (a?.idpacienteagendamento && String(a.idpacienteagendamento) === String(vismedAppointmentId)) {
                        return isVismedConsultation(a) ? 'confirmed' : 'unverified';
                    }
                }
            }
    ...
    private async verifyVismedAppointmentByOfficialIdContract(
        ...
        try {
            ...
            const validMatchingResponse = response.every(item => {
                ...
                return returnedId === expectedId && isVismedConsultation(item);
            });
    ...
    private async cancelSyncRecord(clinicId: string, syncRecord: any, reason?: string) {
        ...
        if (syncRecord.doctoraliaBreakId) {
            try {
                ...
                await this.prisma.bookingSync.update({ where: { id: syncRecord.id }, data: {
                    syncedToDoctoralia: false, syncError: String(err.message).slice(0, 500),
                } });
            }
        }
