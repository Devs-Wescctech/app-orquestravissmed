...
async function testUpdate() {
    try {
        ...
        if (facilities._items && facilities._items.length > 0) {
            ...
            if (doctors._items && doctors._items.length > 0) {
                const docId = doctors._items[0].id;
                console.log(`Testing doctor ${docId}`);
                ...
                if (addrs._items && addrs._items.length > 0) {
                    ...
                    console.log(`PUT ${addrUrl}`);
                    const putRes = await fetch(addrUrl, {
                        method: 'PUT',
                        headers,
                        body: JSON.stringify({ name: 'Test Name Updated', city: 'Test City' })
                    });
                    ...
                    if (putRes.status === 405) {
                        ...
                        const shortUrl = `https://www.${DOMAIN}/api/v3/integration/addresses/${addrId}`;
                        console.log(`PUT ${shortUrl}`);
                        const shortPutRes = await fetch(shortUrl, {
                            method: 'PUT',
                            headers,
                            body: JSON.stringify({ name: 'Test Name Updated SHORT' })
                        });
                        console.log('SHORT PUT Response Status:', shortPutRes.status);
                    }
                }
            }
        }
