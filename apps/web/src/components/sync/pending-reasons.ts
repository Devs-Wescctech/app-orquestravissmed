...
export function explainPendingReasons(events: readonly PendingEvent[]) {
    const groups = [
        ...
        { actions: ['managed_scope_pending', 'professional_cleanup_pending'], title: 'Remoção de horários aguardando conferência',
            meaning: 'A limpeza não foi concluída ou confirmada. Pode haver histórico incompleto, vínculo compartilhado, reserva, bloqueio ou divergência na agenda. Consulte o evento para distinguir uma remoção não enviada de um resultado ainda incerto.',
