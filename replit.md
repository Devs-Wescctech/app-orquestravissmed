...
- **Insurance matching engine**: Uses NON_INSURANCE_PATTERNS to skip payment-type entries (orcamento, cartao clinica, estetica, google a faturar, etc.), token-based matching with combined score (60% token overlap + 40% dice similarity). Only exact string match auto-links; contains match goes to PENDING_REVIEW at ~90%, token match at 55%+, fuzzy match at 65%+ (must share at least one core token). Rescan step auto-cleans false-positive matches when a VisMed insurance name matches NON_INSURANCE_PATTERNS.
- **Profissionais tab**: Shows sync status indicator (Completo/Parcial/Sem Turnos/Pendente) instead of manual sync button — sync is fully automatic via global pipeline.
