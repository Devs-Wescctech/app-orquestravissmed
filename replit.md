...
- **SlotSyncService** (`slot-sync.service.ts`): Converts turnoM/T/N into Doctoralia calendar slots via `replaceSlots` API. Generates slots for next 30 days per address with services. Unified mapping selection is clinic-scoped when clinicId is provided. Auto-provisions address services from specialty mappings when no services exist on an address. Uses a candidate fallback mechanism: tries mapped `doctoraliaServiceId` first (preserving priority order), then alternatives with same normalizedName, stopping on success or non-retryable errors (401/403/429/5xx). Error status uses structured `error.status` with regex fallback.
