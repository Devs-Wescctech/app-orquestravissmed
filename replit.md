...
- **SlotSyncService** (`slot-sync.service.ts`): Converts turnoM/T/N into Doctoralia calendar slots via `replaceSlots` API. Generates individual 30-min slot windows (e.g., 08:00-08:30) for next 30 days per address. Deduplicates address services by `service_id` to avoid sending duplicate entries. Auto-enables calendar (`booking_type=integration`) before sending slots; skips address on 4xx enable failure, proceeds on transient errors. Auto-provisions address services from specialty mappings when no services exist. Uses candidate fallback mechanism for service provisioning with structured error handling.
