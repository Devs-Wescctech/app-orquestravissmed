...
- **Insurance payload (CRITICAL — confirmed via official docs + UI testing 24/Apr/2026)**:
  - `POST/PUT /addresses/{id}/insurance-providers` payload is ONLY `{insurance_provider_id: N}` — NO `insurance_plans` field (legacy/invented field; API silently ignored it). Returns 204.
  - Slot payload MUST use `insurance_accepted: "with-insurance-only"` + `insurance_providers: [N]` for the UI to display the slots as bookable with insurance. `with-and-without-insurance` was confirmed to make the UI show "Não tem vagas online com convênio" — Doctoralia treats those slots as private-only by default. The 3 valid enum values are `with-insurance-only` (default), `with-and-without-insurance`, `without-insurance-only`.
